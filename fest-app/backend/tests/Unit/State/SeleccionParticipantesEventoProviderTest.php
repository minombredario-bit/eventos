...
class SeleccionParticipantesEventoProviderTest extends TestCase
{
    public function testProvideBuildsResponseFromGranularSelection(): void
    {
        ...
        $actividad->method('getId')->willReturn('actividad-1');
        ...
        $lineaActiva->method('getActividad')->willReturn($actividad);
        $lineaActiva->method('getNombreActividadSnapshot')->willReturn('Menú especial');
        ...
        $this->assertSame('actividad-1', $response->participantes[0]['inscripcionRelacion']['lineas'][0]['actividadId']);
    ...
    public function testProvideReturnsEmptyResponseWhenEventoHasNoActiveActividades(): void
    {
        $entidad = $this->createMock(Entidad::class);
        $entidad->method('getId')->willReturn('entidad-1');
