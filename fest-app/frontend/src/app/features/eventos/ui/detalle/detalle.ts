...
export class Detalle {
  ...
  protected readonly inscritosRows = computed<FamilyMember[]>(() => {
    ...
    for (const inscrito of this.inscritos()) {
      ...
      const actividadNames = activeLines
        .map((linea) => linea.nombreActividadSnapshot.trim())
        .filter(Boolean)
        .join(', ');
      ...
      rows.push({
        ...
        notes: actividadNames ? `Actividades: ${actividadNames}` : undefined,
  ...
  protected readonly eventStatusLabel = computed(() => {
    ...
    const fechaInicioInscripcion = this.toValidDate(event.fechaInicioInscripcion);
    const fechaLimiteInscripcion = this.toValidDate(event.fechaLimiteInscripcion);

    if (event.inscripcionAbierta === true) {
      if (fechaLimiteInscripcion && fechaLimiteInscripcion < now) {
        return 'Cerrado';
      }

      if (event.fechaLimiteInscripcion) {
        return `Abierta hasta el ${formatLocalDate(event.fechaLimiteInscripcion, {
          day: 'numeric',
          month: 'long',
        })} a las ${formatTime(event.fechaLimiteInscripcion)}`;
      }

      return 'Abierta';
    }

    if (fechaInicioInscripcion && fechaInicioInscripcion > now && event.fechaInicioInscripcion) {
      return `Se abrirá el ${formatLocalDate(event.fechaInicioInscripcion, {
        day: 'numeric',
        month: 'long',
      })} a las ${formatTime(event.fechaInicioInscripcion)}`;
    }

    if (fechaLimiteInscripcion && fechaLimiteInscripcion < now) {
      return 'Inscripción cerrada por fin de plazo';
    }
  ...
  protected readonly hasActiveActividades = computed(() => this.hasActiveActividadesFromEvent(this.event()));
  ...
  protected readonly canManageParticipants = computed(() =>
    Boolean(this.event()?.inscripcionAbierta === true && this.hasActiveActividades()),
  );
  protected readonly activeActividadCompatibilityScope = computed(() => {
    const event = this.event();
    if (!event || !Array.isArray(event.actividades)) {
      return { allowsAdult: false, allowsInfantil: false };
    }

    const activeActividades = event.actividades.filter((actividad) => actividad.activo !== false);
    if (!activeActividades.length) {
      return { allowsAdult: false, allowsInfantil: false };
    }
    ...
    for (const actividad of activeActividades) {
      if (actividad.compatibilidadPersona === 'ambos') {
        allowsAdult = true;
        allowsInfantil = true;
        continue;
      }

      if (actividad.compatibilidadPersona === 'adulto') {
        allowsAdult = true;
      }

      if (actividad.compatibilidadPersona === 'infantil') {
        allowsInfantil = true;
      }
    }
    ...
    for (const linea of inscripcion.lineas) {
      ...
      if (!current) {
        rows.set(key, {
          ...
          notes: linea.nombreActividadSnapshot.trim()
            ? `Actividades: ${linea.nombreActividadSnapshot.trim()}`
            : undefined,
        });
        continue;
      }

      const nextActividad = linea.nombreActividadSnapshot.trim();
      if (!nextActividad) {
        continue;
      }
      ...
      const existingActividades = existingNote.startsWith(prefix)
        ? existingNote.slice(prefix.length).split(',').map((item) => item.trim()).filter(Boolean)
        : [];

      if (!existingActividades.includes(nextActividad)) {
        current.notes = `${prefix}${[...existingActividades, nextActividad].join(', ')}`;
      }
    }
  ...
  private toValidDate(rawDate?: string | null): Date | null {
    if (!rawDate) {
      return null;
    }
  ...
  protected readonly eventoYaPasado = computed(() => {
    const event = this.event();
