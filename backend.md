...
2. Si esDePago = true:
   a. Si tipoActividad = INFANTIL → aplicar precioInfantil (sin importar tipo de persona)
   b. Si tipoActividad = ADULTO o ESPECIAL:
...
{
    public function calcularPrecio(ActividadEvento $actividad, PersonaFamiliar $persona): float
    {
        ...
        if ($esInterno) {
            return (float) ($actividad->getPrecioAdultoInterno() ?? $actividad->getPrecioBase());
        }
...
- validar que cada actividad pertenece al evento y está activo
...
```php
// src/Service/Reporte/ReporteActividadService.php
...
public function generarExcel(Evento $evento): Response
{
    ...
    // Cabecera
    $sheet->fromArray(['Actividad', 'Adultos', 'Infantiles', 'Total'], null, 'A1');

    // Datos agrupados por actividad
    // ...

    $writer = new Xlsx($spreadsheet);
    $response = new StreamedResponse(fn() => $writer->save('php://output'));
    $response->headers->set('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    $response->headers->set('Content-Disposition', 'attachment; filename="reporte-actividad.xlsx"');
...
- `ActividadAFixtures` — 2-3 actividades por evento
