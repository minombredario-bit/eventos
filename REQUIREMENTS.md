...
- solicitar registro con el código de su entidad
...
- 1:N con Usuario
- 1:N con Evento
- 1:N con CensoEntrada
- N:M con Usuario (admins de la entidad)

### 3.2 CensoEntrada

Tabla donde se vuelca el Excel subido por el superadmin. Sirve para prevalidar registros automáticamente.
...
| codigoRegistroUsado | string | el código que introdujo al registrarse |
| censadoVia | enum CensadoViaEnum | EXCEL / MANUAL / INVITACION |
| censoEntradaRef | FK CensoEntrada | nullable |
...
- un usuario debe introducir el código de entidad antes de registrarse
- el código identifica la entidad y muestra su nombre y logo
- el backend busca coincidencia en `CensoEntrada` por email (primero) o DNI (alternativo)
- si hay coincidencia única → validado automáticamente, `esCensadoInterno: true`, `tipoUsuarioEconomico: INTERNO`
- si hay coincidencia múltiple → pendiente de revisión manual
- si no hay coincidencia → pendiente de validación manual por el admin
- los usuarios pendientes de validación, rechazados o bloqueados no pueden iniciar sesión
...
```
POST   /api/registro/validar-codigo           Valida código de entidad y devuelve nombre, logo y tipo
POST   /api/registro/solicitud                Registro de nuevo usuario (incluye codigoEntidad)
...
PATCH  /api/me                                Edita datos propios
```
...
POST   /api/superadmin/entidades/{id}/censo/importar   Sube Excel de censo
GET    /api/superadmin/entidades/{id}/censo      Lista entradas del censo
GET    /api/superadmin/entidades/{id}/censo/sin-vincular  Entradas sin usuario
