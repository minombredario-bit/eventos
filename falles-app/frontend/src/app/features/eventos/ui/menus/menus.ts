...
export class Menus {
  ...
  protected readonly selectionSummary = computed<SelectionSummaryRow[]>(() => {
    ...
    for (const member of this.membersInScope()) {
      for (const slot of this.getAvailableSlotsForMember(member)) {
        const selectedId = this.selectedMenus()[this.participantKey(member.id, member.origin)]?.[slot];
  ...
  protected updateMenu(payload: MenuChangePayload): void {
    ...
    const nextMenuId = payload.menuId;
    if (previousMenuId === nextMenuId) return;

    this.setSelectedMenuByParticipant(payload.memberId, payload.memberOrigin, slot, nextMenuId);
  ...
  protected getSelectedMenu(member: FamilyMember, slot: MealSlot): string | null {
    return this.selectedMenus()[this.participantKey(member.id, member.origin)]?.[slot] ?? null;
  }

  private getSelectedMenuByParticipant(memberId: string, origin: ParticipantOrigin, slot: MealSlot): string | null {
    return this.selectedMenus()[this.participantKey(memberId, origin)]?.[slot] ?? null;
  }

  private setSelectedMenuByParticipant(
    memberId: string,
    origin: ParticipantOrigin,
    slot: MealSlot,
    menuId: string | null,
    ...
    this.selectedMenus.update((state) => ({
      ...state,
      [memberKey]: { ...(state[memberKey] ?? {}), [slot]: menuId },
    }));
  }
  ...
  protected removeMemberFromMenus(member: FamilyMember): void {
    ...
    this.selectedMenus.update((state) => ({ ...state, [key]: {} }));
  }
    ...
    this.eventosApi
      ...
      .subscribe({
        next: (evento) => {
          ...
          if (this.shouldFallbackToMenusEndpoint(evento, id)) {
    ...
    const hydrationKey = references
      .map((participant) => {
        const lines = (participant.relationLines ?? [])
          .map((line) => `${line.lineId}:${line.menuId}:${line.slot}:${line.estadoLinea}`)
          .join(',');
    ...
    this.selectedMenus.update((state) => {
      ...
      for (const participant of references) {
        ...
        for (const line of lines) {
          ...
          if (!availableInSlot?.has(line.menuId)) {
            continue;
          }
          ...
          next[key] = { ...currentByMember, [line.slot]: line.menuId };
  ...
  private hydrateSelectionsFromExistingInscription(): void {
    ...
    this.selectedMenus.update((state) => {
      ...
      for (const linea of inscripcion.lineas) {
        ...
        const menuId = linea.menuId?.trim();
        if (!slot || !menuId) {
          continue;
        }

        const availableInSlot = optionsBySlot.get(slot);
        if (!availableInSlot?.has(menuId)) {
          continue;
        }
        ...
        next[key] = { ...currentByMember, [slot]: menuId };
        hasHydratedAtLeastOneLine = true;
      }
  ...
  private shouldFallbackToMenusEndpoint(evento: EventoDetalleApi, eventId: string): boolean {
    if (shouldUseLegacyMenusFallback(evento)) {
      return true;
    }
  ...
  private isInfantOnlySlot(slot: MealSlot): boolean {
    const slotMenus = this.options().filter((menu) => menu.slot === slot);
    return slotMenus.length > 0 && slotMenus.every((menu) => menu.compatibility === 'infantil');
  }
        ...
        return {
          ...
          relationLines: (item.inscripcionRelacion?.lineas ?? [])
            ...
            .map((linea) => ({
              ...
              menuId: String(linea.menuId ?? '').trim(),
              menuLabel: String(linea.nombreMenuSnapshot ?? '').trim(),
              slot: this.normalizeMealSlot(linea.franjaComidaSnapshot),
            ...
            .filter((linea): linea is ParticipantRelationLine =>
              Boolean(linea.lineId && linea.menuId && linea.slot),
            ),
        };
      })
      .filter((p) => {
  ...
  private buildRowsFromSelectionContract(): ExistingInscriptionRowView[] {
    ...
    for (const participant of this.preselectedParticipants()) {
      ...
      const lines = (participant.relationLines ?? [])
        ...
        .map((line) => {
          const menuOption = this.options().find((option) => option.id === line.menuId);
          return {
            id: line.lineId,
  ...
  private buildExistingSelectionMap(): Map<string, { inscripcionId: string; lineId: string; menuId: string }> {
    const keys = new Map<string, { inscripcionId: string; lineId: string; menuId: string }>();

    for (const participant of this.preselectedParticipants()) {
      for (const line of participant.relationLines ?? []) {
        if (line.estadoLinea === 'cancelada') continue;
        keys.set(
          this.buildSelectionKey(participant.id, participant.origin, line.slot, line.menuId),
          { inscripcionId: line.inscripcionId, lineId: line.lineId, menuId: line.menuId },
        );
      }
    }
    ...
    for (const line of fallback.lineas) {
      ...
      const menuId = line.menuId?.trim();
      if (!slot || !menuId) continue;
      ...
      keys.set(
        this.buildSelectionKey(participantId, origin, slot, menuId),
        { inscripcionId: fallback.id, lineId: line.id, menuId },
      );
    }
  ...
  private buildExistingSelectionBySlotMap(
    existing: Map<string, { inscripcionId: string; lineId: string; menuId: string }>,
  ): Map<string, { inscripcionId: string; lineId: string; menuId: string }> {
    const bySlot = new Map<string, { inscripcionId: string; lineId: string; menuId: string }>();
  ...
  private buildSelectionKey(
    ...
    return `${this.participantKey(memberId, memberOrigin)}|${slot}|${menuId}`;
  }
}
