...
describe('Detalle autosave y navegación', () => {
  ...
  it('expone botón Borrar solo para invitados inscritos', () => {
    const fixture = createDetalleFixture();
    const component = fixture.componentInstance as unknown as {
      inscritoActionLabel: (member: FamilyMember) => string;
    };

    expect(component.inscritoActionLabel({
      id: 'nf-1',
      name: 'Invitado',
      role: 'Invitado',
      personType: 'adulto',
  ...
  it('borra invitado inscrito, lo quita de selección visible y persiste', () => {
    const guardarSeleccionParticipantes = jasmine
      .createSpy('guardarSeleccionParticipantes')
      .and.callFake((_: string, participantes: ParticipanteSeleccionApi[]) => of(participantes));
    ...
    const component = fixture.componentInstance as unknown as {
      ...
      inscritos: { set: (value: ParticipanteSeleccionApi[]) => void; (): ParticipanteSeleccionApi[] };
      removeInscritoInvitado: (memberId: string) => void;
    };

    component.selectedMemberIds.set(['invitado:nf-77', 'familiar:10']);
    component.inscritos.set([
      { id: 'nf-77', origen: 'invitado' },
      { id: '10', origen: 'familiar' },
    ] as ParticipanteSeleccionApi[]);

    component.removeInscritoInvitado('nf-77');

    expect(bajaInvitadoEnEvento).toHaveBeenCalledWith('evt-1', 'nf-77');
    expect(component.selectedMemberIds()).toEqual(['familiar:10']);
    expect(component.inscritos()).toEqual([{ id: '10', origen: 'familiar' } as ParticipanteSeleccionApi]);
...
function createDetalleFixture(overrides?: {
  ...
  const eventosApiMock = {
    getEvento,
    getRelacionesByUsuario: jasmine.createSpy('getRelacionesByUsuario').and.returnValue(of([])),
