...
class EventoControllerTest extends TestCase
{
    ...
    public function testInscribirmeUsesActividadWhenActividadAndMenuIdArePresentAndStillFlagsDeprecation(): void
    {
        $entidad = $this->createConfiguredMock(\App\Entity\Entidad::class, ['getId' => 'entidad-1']);
        ...
        $inscripcionService
            ...
            ->with(
                ...
                $this->callback(static fn(array $personas): bool =>
                    ($personas[0]['actividad'] ?? null) === '/api/menu_eventos/actividad-canonic-1'
                    && ($personas[0]['menu_id'] ?? null) === 'legacy-id-1'
                ),
            )
            ->willReturn($inscripcion);
        ...
        $request = Request::create('/api/eventos/evento-1/inscribirme', 'POST', [], [], [], [], json_encode([
            'personas' => [[
                'usuario' => '/api/usuarios/user-1',
                'actividad' => '/api/menu_eventos/actividad-canonic-1',
                'menu_id' => 'legacy-id-1',
            ]],
        ], \JSON_THROW_ON_ERROR));
