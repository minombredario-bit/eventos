...
export interface ActividadEvento {
  ...
  nombre: string;
  descripcion?: string;
  tipoActividad: 'adulto' | 'infantil' | 'especial' | 'libre';
  esDePago: boolean;
  precioBase: number;
...
export interface InscripcionRequest {
  personas: {
    ...
    actividad: string;       // IRI: /api/actividad_eventos/{id}
    observaciones?: string;
  }[];
}
```
...
export class SelectorPersonasComponent {
  ...
  precioEstimado(linea: LineaSeleccion): number {
    if (!linea.asiste || !linea.actividadId) return 0;
    const actividad = this.actividades.find(m => m.id === linea.actividadId);
    if (!actividad || !actividad.esDePago) return 0;
    // Precio orientativo; el backend recalcula
    return actividad.precioBase;
  }
}
```
